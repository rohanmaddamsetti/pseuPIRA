...
def main():
    ...
    ## validate that reference genome file exists
    assert os.path.exists(args.reference) ## we assume that reference file in right format
    ...
    for cur_fastq_path in args.reads:
        ...
        assert cur.fastq_path.endswith(".fastq") or cur.fastq_path.endswith(".fastq.gz"), "ERROR: non-fastq files supplied as arguments"
